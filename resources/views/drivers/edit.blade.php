            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('drivers.update', $driver->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $driver->name)" required autofocus />
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="license_expiry" name="license_expiry" type="date" class="mt-1 block w-full" :value="old('license_expiry', $driver->license_expiry)" required />
                    ...
                    <div class="mb-4">
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            <option value="assigned" {{ old('status', $driver->status) == 'assigned' ? 'selected' : '' }}>Ditugaskan</option>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="fleet_id" name="fleet_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            @foreach ($fleets as $fleet)
                                <option value="{{ $fleet->id }}" {{ old('fleet_id', $driver->fleet_id) == $fleet->id ? 'selected' : '' }}>{{ $fleet->name }}</option>
                            @endforeach
                        </select>
